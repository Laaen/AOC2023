...
i = 2
var next : Point
var points_arr : seq[Point]
points_arr.add(start)
# Startup
...
points_arr.add(start)
var res = 0
for idx in 0..(points_arr.len - 2):
    res += (points_arr[idx].x * points_arr[idx + 1].y) - (points_arr[idx + 1].x * points_arr[idx].y)
echo (abs(res)/2 - i/2 + 1.toFloat).toInt